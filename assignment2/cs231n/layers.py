...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # batch whitening (for each scalar feature)
        xnorm = (x - sample_mean) / np.sqrt(sample_var + eps)

        # scale and shift
        out = gamma * xnorm + beta

        # update running mean and variances (with momentum)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var

        cache = [x, xnorm, gamma, sample_mean, sample_var, eps]

    elif mode == 'test':

        # use running mean and variances for test time
...
def batchnorm_backward(dout, cache):
    ...
    x, xnorm, gamma, sample_mean, sample_var, eps = cache
    N, D = x.shape

# add_gate_1(gamma * xnorm, beta)
    dgamma_xnorm = dout
    dbeta = np.sum(dout, axis=0)

# multiply_gate_1(xnorm, gamma)
    dxnorm = dgamma_xnorm * gamma
    dgamma = np.sum(dgamma_xnorm * xnorm, axis=0)

# multiply_gate_2(x - mean, 1 / sqrt(var + eps))
    dxu1 = dxnorm * (1 / np.sqrt(sample_var + eps))
    dinv_sqrt_var = np.sum(dxnorm * (x - sample_mean), axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dvar = np.sum(-0.5 * dxnorm * xmu * ivar**3, axis = 0)
    dmean = -np.sum(dxnorm * ivar, axis=0) + dvar * -np.sum(2 * xmu / N, axis=0)

    dx = dxnorm * (ivar) + (2./N) * dvar * xmu + dmean / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    if (H1 % 1 != 0):
        raise ValueError('Horizontal stride is not an integer!')
    else:
        H1 = int(H1)

    if (W1 % 1 != 0):
        raise ValueError('Vertical stride is not an integer!')
    else:
        W1 = int(W1)
    ...
    for i in range(N):
        for ifilt in range(F):
            xp = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)),  'constant')
            for ih in range(H1):
                for iw in range(W1):
                    out[i, ifilt, ih, iw] = np.sum(xp[i, :, ih*stride:ih*stride + HH,
                                                            iw*stride:iw*stride + WW] *
                                                            w[ifilt, :, :, :])
                    out[i, ifilt, ih, iw] += b[ifilt]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for ic in range(C):
            for ih in range(H1):
                for iw in range(W1):
                    out[i, ic, ih, iw] = np.max(x[i, ic, ih*stride:ih*stride + HH,
                                                         iw*stride:iw*stride + WW])
